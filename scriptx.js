...
document.getElementById("indicator1Dropdown").addEventListener("change", () => {
  graphDraw(rawData); // Call graphDraw with rawData
});
...
// Columns we want to present in the HTML table
const columnsOrder = [
  "Description",
  "Indicator1",
  "Indicator2",
...
function populateIndicator1Dropdown(data) {
  ...
  indicator1Values.forEach((value) => {
    ...
    option.value = value;
    option.text = value;
    option.selected = true; // By default, all options are selected
...
// Function to filter data based on the selected Indicator1 values
...
function renderTable(data, elementId, columnsOrder) {
